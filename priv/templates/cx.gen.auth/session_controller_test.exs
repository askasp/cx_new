defmodule <%= inspect context.web_module %>.<%= inspect Module.concat(schema.web_namespace, schema.alias) %>SessionControllerTest do
  ...
  describe "GET <%= web_path_prefix %>/users/log_in" do
    ...
    test "redirects if already logged in", %{conn: conn, user: user} do
      conn = conn |> log_in_user(user) |> get(Routes.<%= schema.route_helper %>_session_path(conn, :new))
      assert redirected_to(conn) == "/"
    end
  end

  describe "POST <%= web_path_prefix %>/users/log_in" do
    ...
    test "logs the user in with remember me", %{conn: conn, user: user} do
      ...
      assert conn.resp_cookies["_<%= web_app_name %>_user_remember_me"]
      assert redirected_to(conn) =~ "/"
    end
  ...
  describe "DELETE <%= web_path_prefix %>/users/log_out" do
    test "logs the user out", %{conn: conn, user: user} do
      conn = conn |> log_in_user(user) |> delete(Routes.<%= schema.route_helper %>_session_path(conn, :delete))
      assert redirected_to(conn) == "/"
